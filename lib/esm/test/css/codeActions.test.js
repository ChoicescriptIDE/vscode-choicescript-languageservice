...
import { getCSSLanguageService, TextDocument, Range, TextDocumentEdit } from '../../cssLanguageService';
suite('CSS - Code Actions', function () {
    var testCodeActions = function (value, tokenBefore, expected) {
        var ls = getCSSLanguageService();
        ...
        var range = Range.create(startPosition, endPosition);
    ...
    var assertCodeAction = function (commands, document, expected) {
        ...
        for (var _i = 0, expected_1 = expected; _i < expected_1.length; _i++) {
            ...
            assert.equal(TextDocument.applyEdits(document, command.arguments[2]), exp.content);
