...
import { TextDocument, getCSSLanguageService } from '../../cssLanguageService';
function assertRanges(lines, expected, languageId, rangeLimit) {
    ...
    var document = TextDocument.create("test://foo/bar." + languageId, languageId, 1, lines.join('\n'));
    var context = rangeLimit ? { rangeLimit: rangeLimit } : {};
    var actualRanges = getCSSLanguageService().getFoldingRanges(document, context);
