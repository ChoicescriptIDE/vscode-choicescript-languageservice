...
import { Scanner, TokenType } from '../../parser/ChoiceScriptScanner';
import { type } from 'os';
suite('ChoiceScript - Scanner', function () {
    function assertSingleToken(scan, source, len, offset, text) {
        ...
        assert.equal(token.type, tokenTypes[0], "Expected type '" + type + "' but got '" + token.type + "'\n\tE: '" + text + "', G: '" + token.text + "'");
    ...
    test('Indentation', function () {
        ...
        // #Option 1
        { len: 3, offset: 10, text: '\t\t\t', type: TokenType.Indentation }, { len: 1, offset: 13, text: '#', type: TokenType.Hash }, { len: 6, offset: 14, text: 'Option', type: TokenType.Ident }, { len: 1, offset: 21, text: '1', type: TokenType.Num }, { len: 1, offset: 22, text: '\n', type: TokenType.EOL }, 
        // Text 1
        { len: 4, offset: 23, text: '\t\t\t\t', type: TokenType.Indentation }, { len: 4, offset: 27, text: 'Text', type: TokenType.Ident }, { len: 1, offset: 32, text: '1', type: TokenType.Num }, { len: 1, offset: 33, text: '\n', type: TokenType.EOL }, 
        // #Option 2
        { len: 3, offset: 34, text: '\t\t\t', type: TokenType.Indentation }, { len: 1, offset: 37, text: '#', type: TokenType.Hash }, { len: 6, offset: 38, text: 'Option', type: TokenType.Ident }, { len: 1, offset: 45, text: '2', type: TokenType.Num }, { len: 1, offset: 46, text: '\n', type: TokenType.EOL }, 
        // Text 2
        { len: 4, offset: 47, text: '\t\t\t\t', type: TokenType.Indentation }, { len: 4, offset: 51, text: 'Text', type: TokenType.Ident }, { len: 1, offset: 56, text: '2', type: TokenType.Num }, { len: 1, offset: 57, text: '\n', type: TokenType.EOL });
    });
    test('Token Word', function () {
        ...
        assertMultipleTokens(scanner, 'forty-two', { len: 5, offset: 0, text: 'forty', type: TokenType.Ident }, { skip: false, len: 1, offset: 5, text: '-', type: TokenType.Delim }, // If we didn't care about the hyphen, "skip: true" means don't bother trying to match it.
        { len: 3, offset: 6, text: 'two', type: TokenType.Ident });
    });
    ...
    test('Identifiers', function () {
        var scanner = new Scanner();
        assertSingleToken(scanner, 'ident', 5, 0, 'ident', TokenType.Ident);
        assertSingleToken(scanner, 'other_mod+1', 9, 0, 'other_mod', TokenType.Ident, TokenType.Delim, TokenType.Num);
    });
    ...
    test('Comments', function () {
        var scanner = new Scanner();
        assertSingleToken(scanner, '*comment', 8, 0, '*comment', TokenType.SingleLineComment);
    ...
    test('Fairmath Operators', function () {
        var scanner = new Scanner();
        assertSingleToken(scanner, '%-', 2, 0, '%-', TokenType.FairMathSub);
        assertSingleToken(scanner, '%+', 2, 0, '%+', TokenType.FairMathAdd);
    });
});
