...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "assert", "path", "../../parser/cssSymbolScope", "../../parser/cssNodes", "../../languageFacts/facts", "../../cssLanguageService", "vscode-uri", "../testUtil/fsProvider", "../testUtil/documentContext"], factory);
    }
    ...
    exports.newRange = exports.getTestResource = exports.assertScopeBuilding = exports.assertSymbolsInScope = exports.assertColorPresentations = exports.assertColorSymbols = exports.assertSymbols = exports.assertLinks = exports.assertHighlights = exports.assertScopesAndSymbols = void 0;
    ...
    function assertScopesAndSymbols(ls, input, expected) {
    ...
    function assertHighlights(ls, input, marker, expectedMatches, expectedWrites, elementName) {
        var document = cssLanguageService_1.TextDocument.create('test://test/test.css', 'css', 0, input);
        ...
        for (var _i = 0, highlights_1 = highlights; _i < highlights_1.length; _i++) {
            ...
            if (highlight.kind === cssLanguageService_1.DocumentHighlightKind.Write) {
                nWrites++;
            }
    ...
    function assertLinks(ls, input, expected, lang, testUri, workspaceFolder) {
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        document = cssLanguageService_1.TextDocument.create(testUri || "test://test/test." + lang, lang, 0, input);
                        stylesheet = ls.parseStylesheet(document);
                        return [4 /*yield*/, ls.findDocumentLinks2(document, stylesheet, documentContext_1.getDocumentContext(workspaceFolder || 'test://test'))];
                    case 1:
                        links = _a.sent();
    ...
    exports.assertLinks = assertLinks;
    function assertSymbols(ls, input, expected, lang) {
        if (lang === void 0) { lang = 'css'; }
        var document = cssLanguageService_1.TextDocument.create("test://test/test." + lang, lang, 0, input);
        var stylesheet = ls.parseStylesheet(document);
    ...
    function assertColorSymbols(ls, input) {
        var expected = [];
    ...
    exports.assertColorSymbols = assertColorSymbols;
    function assertColorPresentations(ls, color) {
        var expected = [];
        for (var _i = 2; _i < arguments.length; _i++) {
            expected[_i - 2] = arguments[_i];
        }
        var document = cssLanguageService_1.TextDocument.create('test://test/test.css', 'css', 0, '');
        var stylesheet = ls.parseStylesheet(document);
        ...
        assert.deepEqual(result.map(function (r) { return r.textEdit; }), expected.map(function (l) { return cssLanguageService_1.TextEdit.replace(range, l); }));
    }
    exports.assertColorPresentations = assertColorPresentations;
    function assertSymbolsInScope(ls, input, offset) {
        var selections = [];
    ...
    function assertScopeBuilding(ls, input) {
        var scopes = [];
        for (var _i = 2; _i < arguments.length; _i++) {
    ...
    function getTestResource(path) {
        return vscode_uri_1.URI.file(path_1.join(__dirname, '../../../../test/linksTestFixtures', path)).toString();
    }
    exports.getTestResource = getTestResource;
    ...
    function createScope(ls, input) {
        var document = cssLanguageService_1.TextDocument.create('test://test/test.css', 'css', 0, input);
        var styleSheet = ls.parseStylesheet(document), global = new cssSymbolScope_1.GlobalScope(), builder = new cssSymbolScope_1.ScopeBuilder(global);
        assertNoErrors(styleSheet);
        styleSheet.acceptVisitor(builder);
        return global;
    }
    function getCSSLS() {
        return cssLanguageService_1.getCSSLanguageService({ fileSystemProvider: fsProvider_1.getFsProvider() });
    }
    suite('CSS - Navigation', function () {
        ...
        suite('Symbols', function () {
            test('basic symbols', function () {
                var ls = getCSSLS();
                assertSymbols(ls, '.foo {}', [{ name: '.foo', kind: cssLanguageService_1.SymbolKind.Class, location: cssLanguageService_1.Location.create('test://test/test.css', newRange(0, 7)) }]);
                assertSymbols(ls, '.foo:not(.selected) {}', [{ name: '.foo:not(.selected)', kind: cssLanguageService_1.SymbolKind.Class, location: cssLanguageService_1.Location.create('test://test/test.css', newRange(0, 22)) }]);
                // Media Query
                assertSymbols(ls, '@media screen, print {}', [{ name: '@media screen, print', kind: cssLanguageService_1.SymbolKind.Module, location: cssLanguageService_1.Location.create('test://test/test.css', newRange(0, 23)) }]);
            });
        });
        ...
        suite('Color', function () {
            test('color symbols', function () {
                var ls = getCSSLS();
                assertColorSymbols(ls, 'body { backgroundColor: #ff9977; }', { color: facts_1.colorFrom256RGB(0xff, 0x99, 0x77), range: newRange(24, 31) });
                assertColorSymbols(ls, 'body { backgroundColor: hsl(0, 0%, 100%); }', { color: facts_1.colorFrom256RGB(255, 255, 255), range: newRange(24, 40) });
                assertColorSymbols(ls, 'body { backgroundColor: hsl(0, 1%, 100%); }', { color: facts_1.colorFrom256RGB(255, 255, 255), range: newRange(24, 40) });
                assertColorSymbols(ls, '.oo { color: rgb(1,40,1); borderColor: hsl(120, 75%, 85%) }', { color: facts_1.colorFrom256RGB(1, 40, 1), range: newRange(13, 24) }, { color: facts_1.colorFromHSL(120, 0.75, 0.85), range: newRange(39, 57) });
                assertColorSymbols(ls, 'body { backgroundColor: rgba(1, 40, 1, 0.3); }', { color: facts_1.colorFrom256RGB(1, 40, 1, 0.3), range: newRange(24, 43) });
            });
            test('color presentations', function () {
                var ls = getCSSLS();
                assertColorPresentations(ls, facts_1.colorFrom256RGB(255, 0, 0), 'rgb(255, 0, 0)', '#ff0000', 'hsl(0, 100%, 50%)');
                assertColorPresentations(ls, facts_1.colorFrom256RGB(77, 33, 111, 0.5), 'rgba(77, 33, 111, 0.5)', '#4d216f80', 'hsla(274, 54%, 28%, 0.5)');
            });
        });
    });
