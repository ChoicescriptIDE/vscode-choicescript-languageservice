...
suite('ChoiceScript - Scanner', () => {
	...
	function assertMultipleTokens(scan: Scanner, source: string, ...tokens: TokenMatchInfo[]) {
		scan.setSource(source);
		let token;
		let matchIndex = 0;
	...
	test('Token Word', function() {
		...
		assertMultipleTokens(scanner, 'forty-two', 
			{len: 5, offset: 0, text: 'forty', type: TokenType.Ident}, 
			{skip: false, len: 1, offset: 5, text: '-', type: TokenType.Delim }, // If we didn't care about the hyphen, "skip: true" means don't bother trying to match it.
